...
\begin{abstract} Positive semidefinite Hermitian matrices that are not fully specified but whose underlying graph is chordal can be completed to a positive semidefinite Hermitian matrix. For positive definite partial Hermitian matrices this completion can be characterized as the unique completion that maximizes the determinant of the matrix. It is also the unique matrix whose inverse has zeroes in the places that were undetermined in the original matrix. This paper extends these characterizations to the case of semidefinite matrices. Because the determinant vanishes for singular matrices and the inverse does not exist we introduce a generalized determinant and use generalized inverses to formulate equivalent characterizations in the semidefinite case. For a class of matrices that are singular but of maximal rank unique characterizations can be given just as in the positive definite case. 
\end{abstract}
...
The question of which partial Hermitian matrices $H$ can be completed to give a fully specified positive definite Hermitian matrix was solved in \cite{grone}. The solution provided in \cite{grone} gave one element of the completion at a time. While giving the correct solution the practical implementation of this procedure was rather tedious. In \cite{smith} a faster procedure was proposed that delivered the same matrix in far fewer steps by focusing on whole blocks at once. 

	The starting point of both procedures is the graph $\gamma$ that is constructed from the incomplete matrix. The vertices of $\gamma$ are given by the rows $i$, $i=1,\ldots,n$, (or columns) of the matrix $H$. Two vertices $i$ and $j$ of $\gamma$ are connected by an edge $e$ if the element 
	\begin{equation}
		a_{ij}
	\end{equation}
	of the matrix is given. There is now one condition that the graph $\gamma$ has to satisfy for the procedure to work. The graph has to be \emph{chordal}. A graph is chordal if every loop of four or more elements has a chord, i.e. an edge that connects two nonconsecutive elements of the loop (for an introduction to graph theoretic notions see \cite{golumbic}). Assuming that the graph is chordal we now proceed by constructing another graph $\Gamma$. The vertices of $\Gamma$ are given by the cliques of $\gamma$ (a clique is a maximal set of vertices that are all connected to each other; again see \cite{golumbic}) and two vertices of $\Gamma$ are connected if the two corresponding cliques have a nonempty intersection (see figure \ref{fig.allgraphs}).

\begin{figure}[hbt]
  \begin{center}
  	\includegraphics[width=10cm]{img/allGraphs.png}
  \end{center}
  \caption{a) Given an incomplete matrix $H$ we can construct a graph $\gamma$ that has an edge for all the specified elements of the matrix. The vertices of the graph are the rows (or columns) of the matrix. For the completion procedure to work the graph $\gamma$ needs to be chordal. The graph shown here is chordal because the central loop only has three elements. b) The dashed lines show the cliques of $\gamma$. c) The cliques are the vertices of the new graph $\Gamma$. Edges in $\Gamma$ connect cliques with a nonempty intersection.}\label{fig.allgraphs}
\end{figure}

The contribution of \cite{smith} is to show that the completion procedure can be reduced to edges of the graph $\Gamma$. Any edge in $\Gamma$ is given by two cliques $\alpha$ and $\beta$ that have a nonzero intersection $\alpha\cap\beta\ne\emptyset$. Let 
\begin{equation}
...
\begin{equation}\label{eqn.defx}
	X = B C^+ D,
\end{equation}
where $C^+$ denotes the Moore-Penrose inverse of $C$ (see \cite{roger} and \cite{matrixanalysis}). Note that the matrix does not need to be positive definite for the construction to work. It it sufficient for the matrix to be positive \emph{semi}definite. 
...
In this paper show we now make two contributions. The first contribution is a new prove that the $X$ from equation (\ref{eqn.defx}) gives a positive semidefinite completion of $H[\alpha\cup\beta]$. We then show that we can extend the uniqueness results to the semidefinite case. To do this we need to introduce a generalized determinant that gives the determinant of the nonsingular part of the matrix. This determinant does not have many of the nice properties that the usual determinant has. If we restrict our attention to a special class of matrices though we can prove three results that hold for positive definite matrices to the case of positive semidefinite matrices: Fischer's inequality, the Schur determinant lemma, and Banachiewicz's form of the inverse. These three results allow us to extend the uniqueness results to semidefinite matrices. We start by introducing the class of matrices for which our results will be valid. 
...
\begin{definition}
	Let $H$ be a positive semidefinite Hermitian matrix that is partitioned as in equation (\ref{eqn.defh}). We say that $H$ is of \emph{maximal rank} if and only if $N(H) = N(A)\oplus N(C)$\footnote{We should be precise and say that $H$ is positive semidefinite with respect to the partition in equation (\ref{eqn.defh}). We will refrain from doing this and rely on the reader to infer the partition from the context.}. 
\end{definition}
...
\begin{equation}
	H/A = C - B^\star A^+B,
\end{equation}
where $A^+$ is the Moore-Penrose inverse of $A$ (see \cite{roger} and \cite{matrixanalysis}). Note that the choice of generalized inverse does not matter here since $R(B)\subset R(A)$. Emilie Haynsworth introduced the name and showed that the Schur complement possesses many interesting properties \cite{hayns} (see \cite{horn} for more details). In \cite{schur} Issai Schur showed that for a positive definite $H$ the determinant of $H$ satisfies
\begin{equation}
	\det H = \det A\; \det H/A.
\end{equation}
We want to show now that this equality holds also for the generalized determinant. Again, we focus our attention on $R(A)\oplus R(C)$ where $H$ is positive definite. Using the notation from the previous section we obtain:
\begin{align}
	\detp H & = \det \bar H \\
...
\begin{equation}\label{eqn.defnewh}
	...
	\end{pmatrix}
\end{equation}
To denote submatrices of $H$ we use the notation from \cite{matrixanalysis}. For a set $\alpha\subset \{1, \ldots, n\}$ we let
\begin{equation}
	H[\alpha]
\end{equation}
...
I would like to thank Patrick B\"uchel for his support during the creation of this work as well as Horst K\"ohler and Thomas Streuer for their initial push to look into maximal determinant completions of matrices. 
